...
/*
 * 自动分析捕获文章内容和标题(不是十分准确)
 * 
   $text = file_get_contents($url);
   $obj = new Readability($text);
   print_r($obj->getContent());
 *
 * @Author: Anonymous
 * @Date: 2019-10-10 13:23:37 
 * @Last Modified by: lovefc
 * @Last Modified time: 2019-10-10 13:30:07
 */
...
class Capture
{
    ...
    // 当判定失败时显示的内容
    const MESSAGE_CAN_NOT_GET = "无法分析此页面的内容。";
    ...
    function __construct($source, $input_char = "utf-8")
    {
        ...
        try {
            ...
            // 会有些错误信息，不过不要紧 :^)
            if (!@$this->DOM->loadHTML('<?xml encoding="' . Capture::DOM_DEFAULT_CHARSET . '">' . $source)) {
            ...
            // insert proper
            $this->DOM->encoding = Capture::DOM_DEFAULT_CHARSET;
        } catch (\Exception $e) {
            die($e->getMessage());
        }
    }
    ...
    private function getTopBox()
    {
        ...
        while ($paragraph = $allParagraphs->item($i++)) {
            ...
            $contentScore = intval($parentNode->getAttribute(Capture::ATTR_CONTENT_SCORE));
            ...
            // 保存父元素的判定得分
            $parentNode->setAttribute(Capture::ATTR_CONTENT_SCORE, $contentScore);
        ...
        for ($i = 0, $len = sizeof($this->parentNodes); $i < $len; $i++) {
            ...
            $contentScore    = intval($parentNode->getAttribute(Capture::ATTR_CONTENT_SCORE));
            $orgContentScore = intval($topBox ? $topBox->getAttribute(Capture::ATTR_CONTENT_SCORE) : 0);
    ...
    public function getContent()
    {
        ...
        if ($ContentBox === null)
            throw new Runtime\Exception(Capture::MESSAGE_CAN_NOT_GET);
        ...
        $content = mb_convert_encoding($Target->saveHTML(), Capture::DOM_DEFAULT_CHARSET, "HTML-ENTITIES");

        // 多个数据，以数组的形式返回
        return array(
            'lead_image_url' => $this->getLeadImageUrl($Target),
            'word_count' => mb_strlen(strip_tags($content), Capture::DOM_DEFAULT_CHARSET),
